...
def print_simulation_results(file=None):
    if file is not None:
        ...
        if not result:
            logger.warning("No aggregated metric parquet files found under %s", root)
            return
        latest = max(result, key=lambda item: item.stat().st_ctime)
        df = pd.read_parquet(latest)
    final_score = df[df["scenario"] == "final_score"]
    if final_score.empty:
        logger.warning("No final_score row found in aggregated metrics")
        return
